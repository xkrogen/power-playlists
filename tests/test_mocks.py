...
class MockClient(spotipy.Spotify):
    def __init__(self, track_uri_list, other_playlists: List[Dict[str, str]] = None):
        ...
        self.playlists = [
            testutil.create_playlist_dict(
                "test_pl_uri", [testutil.create_track_dict(uri) for uri in track_uri_list], "test_pl"
            )
        ]
    ...
    def playlist_items(
        self, playlist_id, fields=None, limit=100, offset=0, market=None, additional_types=("track", "episode")
    ):
    ...
    def playlist_remove_specific_occurrences_of_items(self, uri, removal_dict_list, snapshot_id=None):
        ...
        for uri, pos in removal_tuples:
            ...
            if items[pos_corrected]["track"]["uri"] != uri:
                raise ValueError(
                    f"Found uri <{items[pos_corrected]['track']['uri']}> at position <{pos_corrected}> "
...
class TestMockClient:
    @pytest.mark.parametrize(
        "range_start,insert_before,expected_outputs",
        [
            (1, 1, "t1,t2,t3,t4"),
            (1, 2, "t1,t2,t3,t4"),
            (1, 0, "t2,t1,t3,t4"),
    ...
    @pytest.mark.parametrize(
        "position,new_items,expected_outputs",
        [
            (3, "t4", "t1,t2,t3,t4"),
            (0, "t4", "t4,t1,t2,t3"),
            (1, "t4", "t1,t4,t2,t3"),
