...
class AllTracksNode(InputNode):
    ...
    | Property | Type | Required | Description |
    |----------|------|----------|-------------|
    | `include_dynamic` | boolean | optional | Include dynamic playlists when gathering all. Default false. |
    """
...
class OutputNode(NonleafNode):
    ...
    | Property | Type | Required | Description |
    |----------|------|----------|-------------|
    | `input` | string | required | The name of the input node. Single input only; use CombinerNode for multiple. |
    """
...
class CombinerNode(LogicNode):
    ...
    | `combine_type` | string | optional | How to combine inputs: `concat` or `interleave`. Defaults to `concat`. |
    """
...
class SortNode(LogicNode):
    ...
    |----------|------|----------|-------------|
    | `input` | string | required | The name of the input node. Single input only; use CombinerNode for multiple. |
    | `sort_key` | string | required | What to sort by: `time_added`, `name`, `artist`, `album`, or `release_date`. |
    | `sort_desc` | boolean | optional | Sort in descending order if true. Defaults to false. |
    """
...
class FilterEvalNode(FilterNode):
    ...
    |----------|------|----------|-------------|
    | `input` | string | required | The name of the input node. Single input only; use CombinerNode for multiple. |
    | `predicate` | string | required | Python expression. Track available as `t`. Example: `t.popularity > 50`. |
    """
...
class TimeBasedFilterNode(FilterNode, abc.ABC):
    ...
    `AddedAtFilterNode` is used, or release date if `ReleaseDateFilterNode` is used).
    ...
    |----------|------|----------|-------------|
    | `input` | string | required | The name of the input node. Single input only; use CombinerNode for multiple. |
    | `days_ago` | integer | semi-optional | Days prior to current date for cutoff. Relative cutoff. |
    | `cutoff_time` | date-time | semi-optional | ISO 8601 date-time for cutoff. Examples: `2020-01-01`. |
    | `keep_before` | boolean | optional | Keep tracks before cutoff if true, otherwise after. Defaults to false. |
    """
...
class DeduplicateNode(LogicNode):
    ...
    |----------|------|----------|-------------|
    | `input` | string | required | The name of the input node. Single input only; use CombinerNode for multiple. |
    | `use_uris` | boolean | optional | Use full URI for duplicates if true, otherwise name+album+artist. |
    """
...
class DynamicTemplateNode(TemplateNode):
    ...
    | `instances` | sequence of dictionaries | required | Variables for template instances. One copy per dictionary. |
    """
...
class CombineSortDedupOutput(TemplateNode):
    """
    A meta-node combining the logic of `CombinerNode`, `SortNode`, `DeduplicateNode`,
    and `OutputNode`. This is useful for the common case of gathering together a few inputs, removing
    ...
    | Property | Type | Required | Description |
    |----------|------|----------|-------------|
    | `output_playlist_name` | string | required | The name of the output playlist. See `OutputNode`. |
    | `sort_key` | string | required | See `SortNode`. |
    | `input_nodes` | sequence of string | semi-optional | A sequence of node names to use as inputs. |
    | `input_uris` | sequence of string | semi-optional | Playlist URIs for inputs. Format: `spotify:playlist:xxxxxx`. |
    | `sort_desc` | boolean | optional | See `SortNode`. |
    """
