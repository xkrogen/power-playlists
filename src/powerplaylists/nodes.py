...
class OutputNode(NonleafNode):
    ...
    `input` [string] [required]
      ...
      nodes should be combined into a single playlist, use a `CombinerNode`.
    """
...
class FilterEvalNode(FilterNode):
    ...
    `predicate` [string] [required]
      ...
      `spotify_client.Track` object in question is made available as a variable named `t`. For example,
...
class TimeBasedFilterNode(FilterNode, abc.ABC):
    ...
    `input` [string] [required]
      ...
      have one input; if multiple nodes should be combined into a single playlist, use a `CombinerNode`.
    ...
    `cutoff_time` [date-time] [semi-optional]
      An [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) date-time string to use as the cutoff time. This is an
...
class CombineSortDedupOutput(TemplateNode):
    ...
    `output_playlist_name` [string] [required]
      The name of the output playlist. See `OutputNode`.
