...
class SpotifyClient:
    ...
    def current_user_playlists(self, force_reload: bool = False) -> List[PlaylistDescription]:
        if force_reload or not self.current_user_playlists_loaded:
            ...
            self.current_user_playlist_cache = {playlist['uri']: PlaylistDescription(playlist)
    ...
    def create_playlist(
        ...
        new_playlist = Playlist(self.spotipy.user_playlist_create(
            self.current_user().oid, playlist_name, public, collaborative, description), list())
    ...
    def playlist_remove_specific_occurrences_of_items(
            self, playlist_uri: str, removal_tuple_list: List[(str, int)], snapshot_id: Optional[str] = None) -> str:
        tracks_removed = 0
    ...
    def playlist_reorder_items(
            self, playlist_uri: str, range_start: int, insert_before: int, range_length: int = 1,
    ...
    def playlist_add_items(
            self, playlist_uri: str, item_uris: List[str], snapshot_id: Optional[str] = None) -> str:
        tracks_added = 0
    ...
    def playlist_change_details(
            self, playlist_uri: str, name: Optional[str] = None, public: Optional[bool] = None,
